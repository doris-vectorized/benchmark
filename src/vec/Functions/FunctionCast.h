...
#include <vec/Functions/FunctionHelpers.h>
#include <vec/Common/StringBuffer.hpp>
#include <vec/DataTypes/DataTypeFactory.hpp>
...
struct ConvertImpl<DataTypeString, ToDataType, Name> {
    ...
    static void execute(Block & block, const ColumnNumbers & arguments, size_t result, size_t /*input_rows_count*/, Additions additions [[maybe_unused]] = Additions()) {
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        if (to_decimal && arguments.size() != 2)
        {
            throw Exception("Number of arguments for function " + getName() + " doesn't match: passed "
                + std::to_string(arguments.size()) + ", should be 2.",
                ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH);
        }
        else if (arguments.size() != 1 && arguments.size() != 2)
            throw Exception("Number of arguments for function " + getName() + " doesn't match: passed "
                + std::to_string(arguments.size()) + ", should be 1 or 2. Second argument (time zone) is optional only make sense for DateTime.",
                ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH);
