...
void Block::insert(size_t position, const ColumnWithTypeAndName & elem)
{
    if (position > data.size())
        throw Exception("Position out of bound in Block::insert(), max position = "
            + std::to_string(data.size()), ErrorCodes::POSITION_OUT_OF_BOUND);
...
const ColumnWithTypeAndName & Block::safeGetByPosition(size_t position) const
{
    ...
    if (position >= data.size())
        throw Exception("Position " + std::to_string(position)
            ...
            + std::to_string(data.size() - 1)
            // + ", there are columns: " + dumpNames(), ErrorCodes::POSITION_OUT_OF_BOUND);
            + ", there are columns: ", ErrorCodes::POSITION_OUT_OF_BOUND);
    ...
    for (const auto & elem : data)
    {
        ...
        if (rows == -1)
            ...
            throw Exception("Sizes of columns doesn't match: "
                + data.front().name + ": " + std::to_string(rows)
                + ", " + elem.name + ": " + std::to_string(size)
                , ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH);
    }
}
...
Block Block::cloneWithColumns(const Columns & columns) const
{
    ...
    if (num_columns != columns.size())
        throw Exception("Cannot clone block with columns because block has " + std::to_string(num_columns) + " columns, "
                        "but " + std::to_string(columns.size()) + " columns given.", ErrorCodes::LOGICAL_ERROR);
